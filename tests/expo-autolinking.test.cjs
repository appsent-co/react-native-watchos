...
test('module discovery uses the packaged tool independently of the app Expo toolchain', () => {
  const fixture = createFixture({ dependencies: { 'watch-module': '1.0.0' } });
  try {
    const expoRoot = path.join(fixture.root, 'node_modules', 'expo');
    fs.unlinkSync(expoRoot);
    write(
      path.join(expoRoot, 'package.json'),
      JSON.stringify({
        name: 'expo',
        version: '57.0.19',
        dependencies: { 'expo-modules-autolinking': '0.0.0' },
      })
    );
    write(
      path.join(expoRoot, 'node_modules/expo-modules-autolinking/package.json'),
      JSON.stringify({ name: 'expo-modules-autolinking', version: '0.0.0' })
    );
    fixture.addModule('watch-module', moduleConfig('WatchModule', 'WatchPod'));
    assert.deepEqual(
      discoverModules(fixture.root).map((module) => module.name),
      ['watch-module']
    );
  } finally {
    removeFixture(fixture);
