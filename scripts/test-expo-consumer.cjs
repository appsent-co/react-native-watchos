...
const {
  testedExpoVersions,
  defaultExpoVersion,
} = require('../expo-modules/consumer-compatibility.json');
...
function parseArgs(argv) {
  const options = {
    ...
    tarball: null,
    expoVersion: defaultExpoVersion,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--tarball' || argument === '--workdir') {
      ...
      index += 1;
    } else if (argument === '--expo-version') {
      const value = argv[++index];
      if (!testedExpoVersions.includes(value))
        fail(`Expo ${value} is not in the consumer test matrix.`);
      options.expoVersion = value;
...
function assertSafeWorkdir(workdir) {
  ...
  const supportedBuilds = [
    repositoryBuild,
    ...testedExpoVersions.map((version) => `${repositoryBuild}-${version}`),
  ];
  if (!supportedBuilds.includes(resolved) && !generatedTemporary) {
...
function inspectTarball(tarball) {
  ...
  const required = [
    ...
    'package/expo-modules/core/build/ios/Core/AppContext.swift',
    'package/expo-modules/patches/expo-modules-core+57.0.16.patch',
    'package/expo-modules/patches/expo-modules-jsi+57.0.8.patch',
  ...
  if (
    files.some(
      (file) =>
        file.startsWith('package/expo-modules/core/native/') ||
        file.startsWith('package/expo-modules/jsi/native/') ||
        file.endsWith('/generate-overlay.cjs')
    )
  )
    fail('npm tarball retained the retired vendored sources or generator');
...
function prepare(options) {
  ...
  writeJson(path.join(workdir, 'gate.json'), {
    tarball,
    consumerRoot,
    expoVersion: options.expoVersion,
  });
}
...
function verify(options) {
  const { tarball, consumerRoot, expoVersion } = gateState(options);
  const appRequire = createRequire(path.join(consumerRoot, 'package.json'));
  const installedExpo = readJson(
    appRequire.resolve('expo/package.json')
  ).version;
  if (installedExpo !== expoVersion)
    fail(
      `Expected consumer Expo ${expoVersion}; installed ${installedExpo}. Run prepare/install for this matrix case.`
    );
  inspectTarball(tarball);
