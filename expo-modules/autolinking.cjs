...
const AUTOLINKER_VERSION = '57.0.12';
...
function resolveAutolinker(packages) {
  const expoRequire = createRequire(
    path.join(packages.expo.root, 'package.json')
  );
  let manifestPath;
  try {
    manifestPath = expoRequire.resolve('expo-modules-autolinking/package.json');
  } catch (error) {
    fail(
      'Expo SDK 57 did not provide expo-modules-autolinking: ' + error.message
    );
  }
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  if (manifest.version !== AUTOLINKER_VERSION) {
    fail(
      'expo-modules-autolinking must be ' +
        AUTOLINKER_VERSION +
        '; found ' +
  ...
  if (!fs.existsSync(cli))
    fail('expo-modules-autolinking CLI is missing: ' + cli);
  return cli;
}
...
function discoverModules(projectRoot) {
  ...
  const packages = resolveCompatibility(appRoot);
  const results = search(appRoot, resolveAutolinker(packages));
